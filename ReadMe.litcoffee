...
* **Dynamic UI Adaptability:** Universally auto-scales all visual overlays, contour weights, diagnostic dots, and information text displays relative to the dimensions of the input image. (Annotated Image Display Is Currently Disabled. Uncomment Line 298 To Enable)
...
Upon its first execution, the program sets up a standardized localized workspace:
    ...
    ├── Test Images/                          # Drop-zone for raw specimen scans (.png, .jpg, .tiff)
...
$$p_{end}=\arg\max_{p\in\text{contour}}\|p-\text{CoG}\|$$
...
| `CoG_to_Petiole_End_Pixels`| Float | Direct straight-line distance from the center mass to the stem base. |
